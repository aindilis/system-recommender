...
sub Init {
  ...
  if ($args{Method} eq 'CSO2') {
    ...
    my $statement;
    if (exists $conf->{'-a'}) {
      die "-a method is not supported for CSO2 currently\n";
    } elsif (exists $conf->{'-m'}) {
      die "-m method is not supported for CSO2 currently\n";
    } else {
  ...
  print Dumper({Keys => scalar keys %$docs});
...
sub SelectPurposes {
  my $hash = {};
  print Dumper({Purposes => [$purposes->GetSortedValues]});
...
sub DisplaySystem {
  ...
  if ($_ eq $currentsystem) {
    my $txt = $docs->{$currentsystem}->{Contents};
    $txt =~ s/[^[:alnum:]]/ /sg;
    $retval .= "\n".wrap("\t","\t", $txt);
  } else {
